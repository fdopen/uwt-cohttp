...
(** HTTP client and server using the [Uwt] interfaces. *)
...
module Client : sig
  include Cohttp_lwt.S.Client with type ctx = Cohttp_uwt_net.ctx
  ...
  (** [custom_ctx ?ctx ?resolver ()] will return a context that is the
     ...
     This is useful to supply a {!Conduit_uwt.ctx} with a custom
...
module Server : sig
  include Cohttp_lwt.S.Server with module IO = Cohttp_uwt_io
