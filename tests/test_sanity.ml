...
let server =
  ...
  |> List.map const
  |> (fun tests ->
    let unix_only = match Sys.win32 with
    | true -> []
    | false -> [
...
let ts =
  Cohttp_uwt_test.test_server_s server begin fun uri ->
    ...
    let unreadable_file_500 () =
      OUnit2.skip_if Sys.win32 "no windows support for chmod";
      let fname = "unreadable500" in
      Lwt.finalize (fun () ->
        Uwt_io.open_file ~flags:[Uwt.Fs.O_CREAT] ~perm:0o006
          ~mode:Uwt_io.Output fname >>= fun oc ->
        Uwt_io.write_line oc "never read" >>= fun () ->
        Uwt_io.close oc >>= fun () ->
